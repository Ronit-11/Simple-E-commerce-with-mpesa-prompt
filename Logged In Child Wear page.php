...
<html>
	...
	<body class="allpages">
		...
		<div class="images">
			<?php
            $query = "SELECT product.`product_id`, product.`product_name`, product.`product_description`, product.`unit_price`, product.`available_quantity`, productimages.`product_image`FROM `product`INNER JOIN `productimages` ON product.product_id = productimages.product_id WHERE `subcategory_id` LIKE '%3'";

            $result = mysqli_query($conn,$query);
            if(mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_array($result)) {
                    ?>
                    <form method="post" action="Cart Processing.php?&id=<?php echo $row["product_id"]; ?>&page=Logged In Child Wear page.php" class="product">
                    	<div>
